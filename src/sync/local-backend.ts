...
export class LocalBackend implements SyncBackend {
  ...
  async pull(): Promise<UniversalSchema> {
    ...
    const parsed = UniversalSchema.safeParse(JSON.parse(raw));
    if (!parsed.success) {
      consola.error(`Local sync file validation failed: ${parsed.error.message}`);
      return { agents: [], skills: [], prompts: [] };
    }
    const total = parsed.data.agents.length + parsed.data.skills.length + parsed.data.prompts.length;
